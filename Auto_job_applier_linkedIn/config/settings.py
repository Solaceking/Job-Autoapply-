...
# CHANGED TO FALSE FOR PERSISTENT LOGIN - Uses your default Chrome profile where you're logged into LinkedIn
...
# CHANGED TO TRUE - Helps avoid LinkedIn bot detection
...
# LLM fallback settings (opt-in)
# If True, the application may call a configured LLM provider to suggest answers for unmatched questions.
use_llm_fallback = False           # True or False, opt-in feature
# Provider name, e.g., 'openai'. Leave empty to disable provider calls.
llm_provider = ""
# Path to cache file (relative to project root or logs folder) for storing LLM responses
